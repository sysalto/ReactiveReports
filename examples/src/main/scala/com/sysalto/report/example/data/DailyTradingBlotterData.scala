...
object DailyTradingBlotterData {
	...
	case class Account(accountNbr: Long, accounName: String, planType: String, kycNbr: Long, dob: Date, occupation: String, employer: String, bussinessType: String,
	...
	def getData(): Seq[Agent] = {
		...
		for (i <- 1 to 100) yield {
			...
			val accntList = for (j <- 1 to rand.nextInt(50)) yield {
			...
			val tradeList = for (j <- 1 to rand.nextInt(200)) yield {
