...
//    static private java.util.Map<String, Object> chartData = new java.util.HashMap<>();
    ...
    private void reportHeader(Report report) throws Exception {
        ...
        rs.next();
        Map<String, Object> record = JavaUtil.recordToMap(rs);
        rs.close();
        report.nextLine();
        ...
        report.nextLine(2);
        Float y = report.getY();
        report.print(new RCell(new RText(JavaUtil.getRecordValue(record, "name").toString()).bold()).at(10));
        report.nextLine();
        report.print(new RCell(new RText(JavaUtil.getRecordValue(record, "addr1").toString())).at(10));
        report.nextLine();
        report.print(new RCell(new RText(JavaUtil.getRecordValue(record, "addr2").toString())).at(10));
        report.nextLine();
        report.print(new RCell(new RText(JavaUtil.getRecordValue(record, "addr3").toString())).at(10));
        report.setYPosition(y);
        report.print(new RCell(new RText("Beneficiary information").bold()).at(500));
        report.nextLine();
        report.print(new RCell(new RText(JavaUtil.getRecordValue(record, "benef_name").toString())).at(500));
        report.nextLine(2);
    }
    ...
    private void summaryOfInvestment(Report report) throws Exception {
        ...
        Source<Map<String, Object>, NotUsed> source = JavaUtil.resultSetToSource(rs);
        ...
        CompletionStage result = (CompletionStage) source.via(new GroupTransform()).runWith(Sink.foreach(
                rec1 -> {
                    if (JavaUtil.isFirstRecord(rec1)) {
                        firstY.set(report.getY());
                    }
                    char cc = (char) (firstChar.get().intValue());
                    Map<String, Object> rec = JavaUtil.getRec(rec1);
                    String fund_name = JavaUtil.getRecordValue(rec, "fund_name");
                    BigDecimal value1 = JavaUtil.getRecordValue(rec, "value1");
                    BigDecimal value2 = JavaUtil.getRecordValue(rec, "value2");
                    ...
                    rrow1.print(report);
                    report.setYPosition(y3 + 5);
                    if (JavaUtil.isLastRecord(rec1)) {
                        report.line().from(10, report.getY()).to(m_change.right(), -1).width(0.5f).draw();
                    } else {
        ...
        trow.print(report);
        float chartHeight = report.getY() - firstY.get();
        report.drawPieChart1("", chartData.get(), m_graphic.left() + 5, firstY.get() + chartHeight, m_graphic.right() -
    ...
    private void changeAccount(Report report) throws Exception {
        ...
        CompletionStage result = (CompletionStage) source.via(new GroupTransform()).runWith(Sink.foreach(
                rec1 -> {
                    Map<String, Object> crtRec = JavaUtil.getRec(rec1);
                    String name = JavaUtil.getRecordValue(crtRec, "name");
                    BigDecimal r_value1 = JavaUtil.getRecordValue(crtRec, "value1");
                    BigDecimal r_value2 = JavaUtil.getRecordValue(crtRec, "value2");
                    BigDecimal r_value3 = JavaUtil.getRecordValue(crtRec, "value3");
                    ...
                    rrow1.print(report);
                    report.setYPosition(y21 + 5);
                    report.line().from(10, report.getY()).to(value3.right(), -1).color(200, 200, 200).
                            lineType(new LineDashType(2, 1)).width(0.5f).draw();
                }), materializer);
        result.toCompletableFuture().get();
        rs.close();
    ...
    private void accountPerformance(Report report) throws Exception {
        ...
        RCell r_value3m = new RCell(new RText(JavaUtil.getRecordValue(record, "value3m").toString())).
                rightAllign().between(value3m);
        RCell r_value1y = new RCell(new RText(JavaUtil.getRecordValue(record, "value1y").toString())).
                rightAllign().between(value1y);
        RCell r_value3y = new RCell(new RText(JavaUtil.getRecordValue(record, "value3y").toString())).
                rightAllign().between(value3y);
        RCell r_value5y = new RCell(new RText(JavaUtil.getRecordValue(record, "value5y").toString())).
                rightAllign().between(value5y);
        RCell r_value10y = new RCell(new RText(JavaUtil.getRecordValue(record, "value10y").toString())).
                rightAllign().between(value10y);
        RCell r_annualized = new RCell(new RText(JavaUtil.getRecordValue(record, "annualized").toString())).
    ...
    private void disclaimer(Report report) throws Exception {
        ...
        report.print(new RCell(new RText(txt)).between(10f, report.pgSize().width() - 10f));
        report.nextLine(2);
    }
